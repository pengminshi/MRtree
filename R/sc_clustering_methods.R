...
sc_clustering.seurat <- function(counts, resolutions, metadata=NULL,
    ...
    obj = Seurat::CreateSeuratObject(counts = counts, project = "seurat object",
    ...
    if (find.variable.features==1){
        obj = Seurat::FindVariableFeatures(obj, selection.method = "mean.var.plot", 
                                           ...
                                           verbose=verbose)
        if(verbose) message('Selected ',length( Seurat::VariableFeatures(object = obj)),
    ...
    obj = Seurat::ScaleData(obj, vars.to.regress=vars.to.regress, verbose=verbose)
    ...
    obj.cluster = Seurat::FindNeighbors(obj, dims = 1:npcs, verbose=verbose)
    obj.cluster = Seurat::FindClusters(obj.cluster, resolution = resolutions, 
                                       algorithm = seurat.graph.algorithm, verbose=verbose)
    seurat.clusters = obj.cluster[[]]
    
    if(build.hierarchical.tree){
        obj.cluster = Seurat::BuildClusterTree(object = obj.cluster, verbose = verbose)
        hc.tree = Seurat::Tool(object = obj.cluster, slot = "BuildClusterTree")
...
create_sce_from_counts <- function(counts, col.data, row.data = NULL, scale.factor=1e4) {
    ...
    logcounts(sceset) <- log2(t(t(counts)/colSums(counts)) * scale.factor + 1)
    # use gene names as feature symbols
    SingleCellExperiment::rowData(sceset)$feature_symbol <- rownames(sceset)
...
#' @param scale.factor scalar sets the scale factor for cell-level normalization
#' @param dimensions integer, UMAP output dimensionality, default 2
...
sc_clustering.umap_kmeans <- function(exprs, Ks, type=c('count', 'log'), 
	estimate.k=FALSE, scale.factor=10000, n.neighbors=30, column.prefix='umapkmeans_'){
	...
	umap.out = uwot::umap(exprs, n_neighbors=n.neighbors, n_components=dimensions, 
	                      pca=50, pca_scale=T)

	labels = sapply(Ks, function(k){
	    adp.out.k = ADPclust::adpclust(umap.out$layout, htype = "amise", centroids="auto", nclust = k)
	    kmeans.out = kmeans(umap.out$layout, umap.out$layout[adp.out.k$centers,], k)
	    kmeans.out$cluster
	})
	colnames(labels) = paste0(column.prefix, 'K',as.character(Ks))
